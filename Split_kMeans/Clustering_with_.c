...
int getNumDimensions(const char* filename)
{
    ...
    //TODO: pit‰isikˆ heitt‰‰ tiedoston alkuun ja disabloida kaikki, vaikeuttaa warningien tutkimista
...
// TODO: refaktoroi k‰ytt‰m‰‰nn resulttia DataPointsin sijaan
...
// calculate MSE of a specific cluster //TODO: mieti voiko toteuttaa j‰rkev‰mmin kuin omana funkkarina joka duplikoi calculateMSE
...
ClusteringResult runKMeans(DataPoints* dataPoints, int iterations, Centroids* centroids, Centroids* groundTruth) //TODO: groundTruth ei tarvita t‰‰ll‰, niin pois?
{
...
void localRepartition(DataPoints* dataPoints, Centroids* centroids, size_t clusterToSplit, bool* clustersAffected)
{
    ...
    /* TODO: Q: Kysy/selvit‰, ett‰ tarvitaanko t‰t‰? Oma oletus on, ett‰ ei tarvita
...
double tentativeMseDrop(DataPoints* dataPoints, Centroids* centroids, size_t clusterLabel, size_t localMaxIterations, double originalClusterMSE)
{
    ...
    // Cleanup TODO: t‰m‰ laajempi vai riitt‰‰kˆ se mik‰ on splitClusterissa?
...
//TODO: "static inline..." sellaisten funktioiden eteen jotka eiv‰t muuta mit‰‰n ja joita kutsutaan? Saattaa tehostaa suoritusta
//TODO: partition on int, mutta se voisi olla size_t. Jotta onnistuu niin -1 alustukset pit‰‰ k‰yd‰ l‰pi
...
int main()
{
    ...
    {        
        ...
        handleMemoryError(centroids.points); //TODO: pit‰isikˆ lopullisesta versiosta kommentoida n‰m‰ pois? Helpottavat l‰hinn‰ debuggausta
		for (int i = 0; i < NUM_CENTROIDS; ++i)
		{
			centroids.points[i].attributes = malloc(numDimensions * sizeof(double));
			handleMemoryError(centroids.points[i].attributes);
            centroids.points[i].dimensions = numDimensions; //TODO tarkista tehd‰‰nkˆ muualla
		}
        ...
        for (int repeat = 0; repeat < MAX_REPEATS; ++repeat)
        {
            ...
            if (result1.mse < bestResult1.mse)
            {
                ...
                for (size_t i = 0; i < dataPoints.size; ++i) //TODO: halutaanko t‰t‰ loopata timerin sis‰ll‰?
                {
        ...
        //random Split
        //freeClusteringResult(&result3); TODO: centroideja ei tallenneta viel‰ t‰h‰n
